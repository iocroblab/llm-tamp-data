...
from envs.constants import ASSETS_DIR, COLOR_FRANKA, FRANKA_Limits, BROWN, TAN, WHITE
...
class PybulletEnv:
    ...
    def create_table(self, name="table", color=WHITE, x=0.1, y=0, z=0.01):  # -0.005):
        table_body = create_box(w=3, l=3, h=0.01, color=color)
        set_point(table_body, [x, y, z])
        self._objects[name] = table_body
    

    def create_table_dinner(self, name="dinner_table", color=BROWN, x=0.5, y=0, z=0.1):  # -0.005):
        table_body = create_box(w=.3, l=.3, h=0.2, color=color)
        set_point(table_body, [x, y, z])
        self._objects[name] = table_body

    def create_basket(self, name="basket", color=WHITE, x=0.6, y=0, z=0.002, w=0.2, l=0.4, h=0.01):
        basket_body = create_box(w=w, l=l, h=h, color=color)
        set_point(basket_body, [x, y, z])
        self._objects[name] = basket_body
    
    def create_obj_from_urdf(self, name, urdf_path, obj_position):
        urdf_body = p.loadURDF(urdf_path, obj_position)
        self._objects[name] = urdf_body
...
class PyBulletRobot:
    ...
    def place(self, object, obstacles, x, y, z, theta, traj=None, play_traj=True):
        ...
        print("new box position is ")
        print(new_box_position)
        ...
        print(f"ee grasp position: {ee_grasp_position}")
        print( self.attachments_robot)
        # rotation_offset = self.rotation_offset_dict[self.last_grasp_direction]
        ee_grasp_orientation = quat
        print(f"ee grasp orientation: {ee_grasp_orientation}")
    ...
    def motion_planning(
        ...
        print(f"the collision outcome is " ,{if_collision})
        print(f"the collision feedback is " ,{feedback})
        ...
        if play_traj:
            ...
            for conf in traj:
                ...
                if len(attachments_robot) > 0:
                    ...
                    pss, orii = multiply(ee_link_pose, ee_link_from_tcp)
                    ps, ori = get_pose(attached)
                    set_pose(attached,(pss,ori))
